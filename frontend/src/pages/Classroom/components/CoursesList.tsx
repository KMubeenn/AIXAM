...
import {
  LuBook, LuUsers, LuExternalLink, LuLoader, LuChevronRight,
  LuCircleCheck, LuClock, LuRefreshCw, LuX, LuSend, LuFileText,
  LuLink, LuDownload, LuShare2, LuClipboardList, LuBrainCircuit,
} from "react-icons/lu";
import {
  useClassroomCourses, useClassroomSubmissions, useFetchSubmissionContent,
  usePushGrade, useClassroomCoursework, useGenerateClassReport,
} from "../../../hooks/useCore";
...
const StateBadge: React.FC<{ state: string }> = ({ state }) => {
  ...
  const { label, cls } = map[state] ?? { label: state, cls: "bg-gray-100 dark:bg-slate-800 text-gray-500" };
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 flex items-center gap-2">
                <LuFileText className="w-4 h-4 text-indigo-500" /> Submission Content
              </h3>
              <button
                onClick={() => fetchContent.mutate({ courseId, courseworkId, submissionId: submission.id })}
              ...
              >
                {fetchContent.isPending ? <LuLoader className="w-3.5 h-3.5 animate-spin" /> : <LuRefreshCw className="w-3.5 h-3.5" />}
                {fetchContent.isPending ? "Fetching..." : "Fetch Content"}
              </button>
            </div>
            {!submission.hasAttachments && (
              <p className="text-sm text-gray-400 italic bg-gray-50 dark:bg-slate-800/50 rounded-xl p-4">No attachments for this submission.</p>
            )}
            {fetchContent.data && (
            ...
            {fetchContent.isError && (
              <div className="text-sm text-red-500 bg-red-50 dark:bg-red-900/20 p-3 rounded-xl">Failed to fetch submission content.</div>
            )}
          </div>
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3 flex items-center gap-2">
              <LuSend className="w-4 h-4 text-emerald-500" /> Push Grade to Google Classroom
            </h3>
            {success ? (
              <div className="flex items-center gap-2 p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200 dark:border-emerald-800">
                ...
                <p className="text-sm font-semibold text-emerald-700 dark:text-emerald-300">Grade synced to Google Classroom!</p>
              </div>
            ) : (
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      await pushGrade.mutateAsync({ courseId, courseworkId, submissionId: submission.id, assignedGrade: parsed });
                      setSuccess(true);
                ...
                >
                  {pushGrade.isPending ? <LuLoader className="w-4 h-4 animate-spin" /> : <LuSend className="w-4 h-4" />}
                  {pushGrade.isPending ? "Syncing..." : "Push Grade"}
                </button>
              </div>
            )}
            {pushGrade.isError && <p className="text-xs text-red-500 mt-2">Failed to push grade. Please try again.</p>}
          </div>

          {submission.assignedGrade !== null && (
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
              <p className="text-xs text-blue-500 font-semibold">Current Assigned Grade</p>
...
const ClassroomReportPanel: React.FC<{
  ...
  const handleDownload = async () => {
    if (graded.length === 0) { alert("Grade at least one student first."); return; }
    try {
      const grades = graded.map((s) => ({ student_name: s.studentName, marks: s.assignedGrade ?? 0, max_marks: 100, feedback: "" }));
      ...
      const res = await generateReport.mutateAsync({
        ...
        gradesData: { grades, class_average: avg, overall_feedback: rubrics ? `Rubric: ${rubrics}` : "" },
      });
      const bytes = new Uint8Array(atob(res.file_base64).split("").map((c) => c.charCodeAt(0)));
      const url = URL.createObjectURL(new Blob([bytes], { type: res.mime_type }));
      Object.assign(document.createElement("a"), { href: url, download: res.filename || `${courseworkName}_report.pdf` }).click();
      URL.revokeObjectURL(url);
    } catch (e: any) { alert(e?.response?.data?.error || e?.message || "Failed to generate report"); }
  };

  const handleCopy = async () => {
    if (graded.length === 0) { alert("Grade at least one student first."); return; }
    setCopying(true);
    const avg = graded.reduce((a, s) => a + (s.assignedGrade ?? 0), 0) / graded.length;
    const text = [`📊 Class Report: ${courseworkName}`, `Class Average: ${avg.toFixed(1)}`, "", "Student Grades:", ...graded.map((s) => `- ${s.studentName}: ${s.assignedGrade}`)].join("\n");
    try { await navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 3000); }
    catch { alert("Clipboard access denied. Use Download PDF instead."); }
  ...
  return (
    <div className="mt-6 border-t border-gray-100 dark:border-slate-800 pt-5 space-y-4">
      <h4 className="text-sm font-semibold text-gray-700 dark:text-slate-300 flex items-center gap-2">
        <LuClipboardList className="w-4 h-4 text-indigo-500" /> Grading Rubrics &amp; Report
      </h4>
      <div>
        <label className="text-xs text-gray-500 dark:text-slate-400 font-medium mb-1 block">
          Grading Rubric / Instructions <span className="font-normal">(optional — included in PDF &amp; AI grading context)</span>
        </label>
        <textarea
          value={rubrics} onChange={(e) => setRubrics(e.target.value)}
        ...
        <p className="text-xs text-gray-400 dark:text-slate-500 mt-1 flex items-center gap-1">
          ...
          For AI grading: fetch content above, then ask the chat agent to "grade these submissions" — rubric will be appended as context.
        </p>
      </div>
      <div className="flex flex-wrap gap-3">
        ...
        >
          {generateReport.isPending ? <LuLoader className="w-4 h-4 animate-spin" /> : <LuDownload className="w-4 h-4" />}
          Download PDF Report
        </button>
        <button
          onClick={handleCopy}
        ...
        >
          {copying ? <LuLoader className="w-4 h-4 animate-spin" /> : <LuShare2 className="w-4 h-4" />}
          {copied ? "✓ Copied!" : "Copy Report Summary"}
        </button>
        {graded.length === 0 && (
          <p className="text-xs text-gray-400 dark:text-slate-500 self-center">Grade at least one student to enable reporting.</p>
        )}
      </div>
    </div>
  );
...
// ── Submissions List ───────────────────────────────────────────────────────────
const SubmissionsList: React.FC<{ courseId: string; courseworkId: string; courseworkName?: string }> = ({ courseId, courseworkId, courseworkName }) => {
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-gray-700 dark:text-slate-300 flex items-center gap-2">
          <LuUsers className="w-4 h-4 text-blue-500" /> {courseworkName || "Submissions"}
        </h3>
        <button onClick={() => refetch()} disabled={isRefetching} className="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors">
          <LuRefreshCw className={`w-4 h-4 ${isRefetching ? "animate-spin" : ""}`} />
        </button>
      </div>

      {selected && <GradeSubmissionModal submission={selected} courseId={courseId} courseworkId={courseworkId} onClose={() => setSelected(null)} />}

      {isLoading ? (
        <div className="space-y-2">{[1,2,3].map((i) => <div key={i} className="h-12 bg-gray-100 dark:bg-slate-800 rounded-xl animate-pulse" />)}</div>
      ) : isError ? (
        <div className="p-4 bg-red-50 dark:bg-red-900/20 text-red-600 rounded-xl text-sm">
          Failed to load: {(error as any)?.response?.data?.error || (error as any)?.message || "Unknown error"}
        </div>
        ...
        <>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {submissions.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {s.hasAttachments
                        ? <span className="inline-flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400"><LuCircleCheck className="w-3.5 h-3.5" /> Yes</span>
                        : <span className="text-xs text-gray-400">None</span>}
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => setSelected(s)} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-600 hover:text-white transition-all">
                        Grade
                      </button>
                    </td>
                  </tr>
          ...
          <ClassroomReportPanel courseworkName={courseworkName || "Coursework"} submissions={submissions} />
        </>
      )}
    </div>
  );
...
const CoursesList: React.FC = () => {
  ...
  const { data: cwData, isLoading: cwLoading } = useClassroomCoursework(selectedCourse?.id ?? null);
  ...
  if (isLoading) return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {[1,2,3].map((i) => <div key={i} className="h-32 bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 animate-pulse" />)}
    </div>
  );

  if (isNotConnected || (isError && courses.length === 0)) return (
    <div className="flex flex-col items-center justify-center py-20 text-center bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800">
      ...
      <p className="text-sm text-gray-400 max-w-sm mb-6">Authorize AIXAM to access your Google Classroom to view courses and submissions.</p>
      <a
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {courses.map((course) => (
          <button
            key={course.id}
            onClick={() => { setSelectedCourse({ id: course.id, name: course.name }); setCourseworkId(""); setCwInput(""); }}
          ...
          >
            <div className="flex items-start gap-3 mb-3">
              ...
              {selectedCourse?.id === course.id && (
                <span className="ml-auto text-xs font-bold text-indigo-600 bg-indigo-100 dark:bg-indigo-900/30 px-2 py-0.5 rounded-full">Selected</span>
              )}
            </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-800 p-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-4">
            {cwLoading ? (
              ...
              <select
                value={courseworkId}
                onChange={(e) => { setCourseworkId(e.target.value); setCwInput(e.target.value); }}
              ...
              >
                ...
                {courseworks.map((cw: any) => <option key={cw.id} value={cw.id}>{cw.title}</option>)}
              </select>
            ) : (
              <>
                <input
                  type="text" placeholder="Enter Coursework ID from Google Classroom"
                  value={cwInput} onChange={(e) => setCwInput(e.target.value)}
                  className="flex-1 h-11 px-4 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-sm outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"
                />
                <button
          ...
          {courseworkId && (
            <SubmissionsList
              ...
              courseworkName={courseworks.find((cw: any) => cw.id === courseworkId)?.title ?? courseworkId}
            />
          )}
        </div>
      )}
