...
const Insights: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (serverError) {
        if (serverError.includes("exhausted") || serverError.includes("quota") || serverError.includes("RESOURCE_EXHAUSTED")) {
          ...
          // If it's a short error, it might be a clean validation message
    ...
    <div className="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-xl shadow-sm p-8 text-white mb-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-indigo-200 mt-1">
            {result
              ? "Your personalized subject analysis and study tips"
              : "Click to analyze your performance with AI"}
          </p>
        </div>
        <button
        ...
        >
          {generateInsights.isPending ? (
            ...
            <>
              ...
              {result ? "Re-generate" : "Generate Insights"}
            </>
          )}
        </button>
      </div>
      ...
      {!result && !generateInsights.isPending && (
        <div className="text-center py-6 text-indigo-200 text-sm">
          Your performance data will be analyzed to group your topics into
          subjects and give you personalized tips.
        </div>
      )}
      ...
      {result && (
        <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {result.subjects.map((s, i) => (
                  <div key={i} className="bg-white/10 rounded-lg p-3">
                    ...
                    <p
                      className={`text-2xl font-bold mt-1 ${s.avg_score >= 75 ? "text-emerald-300" : s.avg_score >= 50 ? "text-yellow-300" : "text-red-300"}`}
                    >
          ...
          {result.subjects.length === 0 && result.insights.length === 0 && (
            <div className="bg-white/10 rounded-lg p-6 text-center">
              <h4 className="font-semibold mb-2">No data available yet</h4>
              <p className="text-sm text-indigo-100">
                Take some mock tests or assignments to get personalized AI
                insights!
              </p>
            </div>
          )}
