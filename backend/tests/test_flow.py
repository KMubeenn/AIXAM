    ...
    try:
        with urllib.request.urlopen(req, data=encoded_data) as response:
            ...
            try:
                ...
                return status, text # Return raw text if not JSON (e.g. streaming response)
...
def run_flow():
    log("=== Starting Functional Test Flow ===\n", CYAN)
    ...
    # NOTE: Chat endpoint expects Form Data (request.POST), NOT JSON body.
    message_data = {
        "session_id": session_id,
        "message": "Hello, this is a test message from the functional test script."
    }
    
    # We use is_json=False, is_form=True for this request
    status, res = make_request("POST", "/api/chat/", message_data, headers=auth_headers, is_json=False, is_form=True)
    
    if status == 200:
        # Response is streaming text, so 'res' will be the full text string here
        log(f"[OK] Chat Response Received:", GREEN)
        print(f"Server replied: {str(res)[:100]}...") # Print first 100 chars
    else:
        log(f"[FAIL] Chat Message Failed: {status} - {res}", RED)
